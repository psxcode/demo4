...
public:
	...
	void SimpleDerivedFunction(int num, char *str) {
		printf("In SimpleDerived. num=%d\n", num);
	}
...
TEST( del, test_main )
{
	Demo::Core::Delegate< void () > noparameterdelegate(&SimpleVoidFunction);

	noparameterdelegate(); // invoke the delegate - this calls SimpleVoidFunction()

	// By default, the return value is void.
	typedef Demo::Core::Delegate< void(int, char *) > MyDelegate;

	// If you want to have a non-void return value, put it at the end.
	typedef Demo::Core::Delegate< int(int, char *) > IntMyDelegate;
	...
	newdeleg = Demo::Core::MakeDelegate(&a, &CBaseClass::SimpleMemberFunctionReturnsInt);
	...
	// global function.
	funclist[6] = Demo::Core::MakeDelegate(&d, &CBaseClass::SimpleVirtualFunction);
	...
	funclist[9] = Demo::Core::MakeDelegate(&c, &CDerivedClass::SimpleDerivedFunction);
	...
	Demo::Core::DelegateComposite< void(int, char *) > delcomp;
