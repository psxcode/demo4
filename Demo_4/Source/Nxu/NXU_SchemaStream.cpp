...
public:
	...
	//--------------------------------//
	SchemaXML( Ogre::DataStreamPtr &istream, SchemaStream *ss )
		: mStream(ss)
		, mDocument(0)
		, mNode(0)
		, mSearch(0)
		...
		, mIsValid(false)
	{
		...
		mDocument = new TiXmlDocument;
		if( mDocument->LoadFile(istream) ) {
			mNode = mDocument;
			preParse(mNode);
		} else {
			reportError("Error parsing file '%s' as XML", istream->getName());
			reportError("XML parse error(%d) on Row: %d Column: %d", mDocument->ErrorId(), mDocument->ErrorRow(), mDocument->ErrorCol());
			reportError("XML error description: \"%s\"", mDocument->ErrorDesc());
			delete mDocument;
			mDocument = 0;
	...
	//--------------------------------//
#else //DBS_NXU_FILESYSTEM
#error
#endif //DBS_NXU_FILESYSTEM
...
#elif DBS_NXU_FILESYSTEM == DBS_NXU_FILESYSTEM_OGRE
SchemaStream::SchemaStream( Ogre::DataStreamPtr &stream, bool isBinary, ACCESS_MODE mode )
	: mCurrentCollection(0)
	, mCurrentScene(0)
	, mCurrentActor(0)
	, mCurrentJoint(0)
	...
	, mOgreStream(stream)
{
	if( mode == AM_RB )
	{
		...
		if( mBinary )
		{
			...
			if ( strcmp(stream,"NXUSTREAM") != 0 ) {
				ok = false;
				reportError("File %s is not a valid binary NxuStream file, missing header", mOgreStream->getName() );
			}
			if ( ok && sdk != NX_SDK_VERSION_NUMBER ) {
				ok = false;
				reportError("This binary file %s was saved out with a different version of the SDK. Saved with SDK%d but running SDK%d", mOgreStream->getName(), sdk, NX_SDK_VERSION_NUMBER );
			}
			if ( ok && uv != NXUSTREAM_VERSION ) {
				ok = false;
				reportError("Even though the file %s was saved with the same SDK version, the version format has changed from %d to %d",mOgreStream->getName(),uv,NXUSTREAM_VERSION);
			}
	...
	{
		mSchemaXML = new SchemaXML(mOgreStream,this);
		if ( !mSchemaXML->mDocument )
		{
			delete mSchemaXML;
			mSchemaXML = 0;
...
#elif DBS_NXU_FILESYSTEM == DBS_NXU_FILESYSTEM_OGRE
		nxu_fseek( mOgreStream, static_cast<long>(start) + sizeof(NxU32) + sizeof(NxI32) );
		nxu_fwrite( &len, sizeof(NxU32), 1, mOgreStream );
		nxu_fseek( mOgreStream, static_cast<long>(end) );
#endif
	}
}
...
#elif DBS_NXU_FILESYSTEM == DBS_NXU_FILESYSTEM_OGRE
							nxu_fseek(mOgreStream, static_cast<long>(loc));
#endif
...
#elif DBS_NXU_FILESYSTEM == DBS_NXU_FILESYSTEM_OGRE
	if( mOgreStream != 0 && size )
	{
		size_t count = nxu_fwrite( mem, size, 1, mOgreStream );
		if( count == 0 )
		{
			invalidate("Write failure saving NxuStream data");
		}
...
void SchemaStream::readMem(void *mem,NxU32 size)
{
	...
	if( mOgreStream != 0 && size )
	{
		size_t count = nxu_fread( mem, size, 1, mOgreStream );
		if( count == 0 )
		{
			invalidate("Write failure saving NxuStream data");
		}
