...
#endif
//------------------------------------------------------------
#if DBS_NXU_FILESYSTEM == DBS_NXU_FILESYSTEM_OGRE
NxuPhysicsCollection* loadCollection( Ogre::DataStreamPtr &istream, NXU_FileType type )
{
	NxuPhysicsCollection *ret = 0;

	bool binary = (type == FT_BINARY);
	SchemaStream ss( istream, binary, SchemaStream::AM_RB );
	if( ss.isValid() )
	{
		ret = new NxuPhysicsCollection;
		ret->load(ss);
	...
	{
		reportError("Failed to open physics data file '%s' for read access", istream->getName());
	}

	return ret;
}
//------------------------------------------------------------
bool saveCollection( NxuPhysicsCollection *c, Ogre::DataStreamPtr &ostream, NXU_FileType type, bool save_defaults, bool cook )
{
	bool ret = false;
	if(c)
	{
		setId(c, ostream->getName().c_str(), 0, false);
		...
		if(binary)gSaveDefaults = true;

		SchemaStream ss(ostream, binary, SchemaStream::AM_WB);
		if( ss.isValid() )
		{
			c->store(ss);
			ret = true;
...
#endif //DBS_NXU_FILESYSTEM == DBS_NXU_FILESYSTEM_NATIVE
//------------------------------------------------------------
