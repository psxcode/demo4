...
//------------------------------------------------------------
/*static*/ bool PhysXCooker::CookConvexMesh( const NxConvexMeshDesc& desc, NxStream& stream )
{
	bool result = false;
	if( InitCooking() )
	{
...
//------------------------------------------------------------
/*static*/ bool PhysXCooker::CookTriangleMesh( const NxTriangleMeshDesc& desc, NxStream& stream )
{
	bool result = false;
	if( InitCooking() )
	{
...
//------------------------------------------------------------
/*static*/ bool PhysXCooker::CookConvexMesh( traits::convex_mesh_desc_cont &mcont, traits::stream_cont &scont )
{
...
//------------------------------------------------------------
/*static*/ bool PhysXCooker::CookTriangleMesh( traits::triangle_mesh_desc_cont &mcont, traits::stream_cont &scont )
{
	_ASSERT( mcont.size() == scont.size() );
	auto mit = mcont.begin(); auto meit = mcont.end();
	auto sit = scont.begin(); auto seit = scont.end();
...
//------------------------------------------------------------
/*static*/ void PhysXCooker::cookOgreMesh(Ogre::MeshPtr mesh, NxStream& outputStream, const Ogre::Vector3 &scale, traits::material_binding_cont &materialBindings)
{
